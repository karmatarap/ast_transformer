...
def test(audio_model, test_loader, args):
    ...
    A_targets = []
    with torch.no_grad():
        for i, (audio_input, labels) in enumerate(test_loader):
            audio_input = audio_input.to(device)
        ...
        stats = f1_score(np.argmax(audio_output,1), np.argmax(target,1), average="macro")

        # save the prediction here
        exp_dir = args.exp_dir
        np.savetxt(exp_dir+'/predictions/predictions_test.csv', audio_output, delimiter=',')
